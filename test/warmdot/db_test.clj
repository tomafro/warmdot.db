...
;; connection can be specified in one of two ways - either by setting 
;; warmdot.db.connection/current, or by using the special :db option. 
;;
;; These are equivalent:
;;
;;     (find-first :users :db database)
;;
;;     (with-bindings {#'warmdot.db.connection/current database}
;;       (find-first :users))
;; 
...
;;
;;     (def find-all (partial db/find-all teams-dataset))
;;     (def find-first (partial db/find-first teams-dataset))
